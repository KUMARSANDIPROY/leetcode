...
public:
    int index=0;
    Node* construct(int pre[],int size,int high)
    {
        if(index==size || pre[index]>high) return NULL;
        
        Node *temp=newNode(pre[index++]);
        temp->left=construct(pre,size,temp->data);
        temp->right=construct(pre,size,high);
        return temp;
    }
    ...
    Node* post_order(int pre[], int size)
    {
        //code here
        Node* root=NULL;
        return construct(pre,size,INT_MAX);
    }
};
